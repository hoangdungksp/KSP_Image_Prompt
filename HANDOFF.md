...
## Status: v0.9.1-r12 — Continue trong chat mới

---

## 🆕 v0.9.1-r12 (2026-05-09) — Connectors + Custom Pose

**Latest patch on top of v0.9.1-r11.** Đọc CHANGELOG.md cho chi tiết. Quick summary:

1. **Connectors PROJECT → CAST cho tất cả modes** (Photos/Film/TVC). Trước đây 3 sections đầu dính sát nhau sau khi bỏ separators ở r10. Nay luôn có dot+line giữa.
2. **Color flow Photos pipeline đúng border colors thực tế**: blue→purple→cyan→gold→coral (cũ: purple→green→purple-light, KHÔNG khớp).
3. **Custom Pose free-text** trong dropdown Pose: `✍️ Tự nhập tư thế (free-text)` ngay sau `⚡ Auto-vary all 100`. Khi pick: textarea VN + nút 🌐 Dịch (Gemini lazy-load, giống Custom Intent). All N shots dùng cùng pose text. Camera angles vẫn vary 12 độc lập.

**Tests**: 63/63 PASS (56 cũ + 7 r12). Build OK 8s.

**Pending Jason verify**:
- Connector layout trên Chrome side panel render đúng (dot+line giữa 5 sections)
- Custom Pose text được Banana Pro respect (paste prompt + verify pose match user-typed text)

---

## Status: v0.9.1 (Released base)
